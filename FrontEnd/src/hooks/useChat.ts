        ...
        async (userData?: { name: string; email: string }) => {
            try {
                ...
                if (chatId) {
                    ...
                    const existingChats = await apiService.getUserChats(
                        userData.email
                    );
                ...
                const channel = await socketService.joinChatChannel(chat.id, {
                    ...
                    onTyping: (typing: TypingStatus) => {
                        ...
                        if (!typing || typeof typing !== "object") {
                            console.error(
                                "useChat: Invalid typing data received:",
                                typing
                            );
                            return;
                        }
                        ...
                        setTypingStatuses((prev) => {
                            console.log(
                                "useChat: Typing status received:",
                                typing
                            );
                    ...
                    onAgentJoined: (agent) => {
                        console.log("useChat: Agent joined:", agent);
                        setCurrentChat((prev) =>
                            prev ? { ...prev, agent } : null
                        );
                    },
                    onAgentLeft: () => {
                        console.log("useChat: Agent left");
                        setCurrentChat((prev) =>
                ...
                if (!channel) {
                    throw new Error("Failed to join chat channel");
                }
